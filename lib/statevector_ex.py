...
class StatevectorEx(QuantumState, TolerancesMixin):
    """Statevector class"""

    def __init__(self, data, dims=None, basis=None):
        ...
        if isinstance(data, (list, np.ndarray)):
        ...
        elif isinstance(data, StatevectorEx):
            ...
            self._data = StatevectorEx.from_instruction(data, basis=basis).data
        else:
    ...
    def conjugate(self):
        ...
        return StatevectorEx(np.conj(self.data), dims=self.dims())
    ...
    def tensor(self, other):
        ...
        """
        if not isinstance(other, StatevectorEx):
            other = StatevectorEx(other)
        ret = copy.copy(self)
    ...
    def inner(self, other):
        ...
        if self.dims() != other.dims():
            raise QiskitError(
                f"StatevectorEx dimensions do not match: {self.dims()} and {other.dims()}."
            )
    ...
    @classmethod
    def from_instruction(cls, instruction, basis=None):
        ...
        init[0] = 1.0
        vec = StatevectorEx(init, dims=instruction.num_qubits * (2,))
        return StatevectorEx._evolve_instruction(vec, instruction, basis=basis)
    ...
    @staticmethod
    def _evolve_instruction(statevec, obj, qargs=None, basis=None):
        ...
        if mat is not None:
            ...
            # of the operator
            return StatevectorEx._evolve_operator(statevec, Operator(mat), qargs=qargs)
        ...
        for instr, qregs, cregs in obj.definition:
            if cregs:
                if instr.name is 'measure' :
                    ...
                    StatevectorEx._evolve_operator(statevec, Operator(mat), qargs=new_qargs)
                else :
                    raise QiskitError(
                        f"Cannot apply instruction with classical registers: {instr.name}"
                    )
            # Get the integer position of the flat register
                ...
                if qargs is None:
                    ...
                    new_qargs = [qargs[qubits[tup]] for tup in qregs]
                StatevectorEx._evolve_instruction(statevec, instr, qargs=new_qargs, basis=basis)
        return statevec
